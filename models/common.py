...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=None, dnn=True):
        ...
        #   ONNX Runtime:           *.onnx_model
        #   OpenCV DNN:             *.onnx_model with dnn=True
        super().__init__()
        w = str(weights[0] if isinstance(weights, list) else weights)
        suffix, suffixes = Path(w).suffix.lower(), ['.pt', '.onnx_model', '.tflite', '.pb', '', '.mlmodel']
        ...
        if jit:  # TorchScript
            ...
            check_requirements(('onnx_model', 'onnxruntime-gpu' if torch.has_cuda else 'onnxruntime'))
            import onnxruntime
